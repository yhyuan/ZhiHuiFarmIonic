...
AutoForm.addInputType('polygonMap', {
  template: 'afPolygonMap',
  valueOut: function() {
    var lat, lng, node;
    node = $(this.context);
...
var initTemplateAndGoogleMaps = function() {
  ...
  if (this.data.value) {
    location = typeof this.data.value === 'string' ? this.data.value.split(',') : [this.data.value.lat, this.data.value.lng];
